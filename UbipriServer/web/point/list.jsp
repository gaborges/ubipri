        ...
        <div class="container">
            ...
            <div class="list">
                    ...
                    <form action="remove.jsp" method="get"  onSubmit="return confirmSubmit()">
                        <tbody>
                            <%
                                ...
                                ArrayList<Point> points = pointDAO.getList();
                                ...
                                for (int i = 0; i < points.size(); i++) {
                                    point = points.get(i);
                                    env = point.getEnvironment();

                            %>
                            <tr>
                                <td class="check"><input  type="checkbox" name="check" value="<%=point.getId()%>" /></td>
                                <td><%=point.getId()%></td>
                                <td><%=point.getLatitude()%></td>
                                <td><%=point.getLongitude()%></td>
                                <td><%=point.getAltitude()%></td>
                                <td><%=point.getOrder()%></td>                            
                                <td id="info">
                                    <%if (env.getId() != null) {%>
                                    <ul>
                                        <li><%=env.getName()%>
                                ...
                                <td><a href="update.jsp?id=<%=point.getId()%>">Update</a></td>
                            </tr>
                            <%}%>
                        </tbody>
                </table>
                ...
                <form name="form_incluir" action="form.jsp" class="inline">
                    <input type="submit" value="New Point" name="new"/>
                </form>
            </div>
        </div>
    </body>
