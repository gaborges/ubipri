...
public class CommunicationTypeDAO {
     ...
     */
    public CommunicationTypeDAO(AccessBD db) {
        this.db = db;
    }
     ...
     * @return
     */
    public void insert(CommunicationType o) {
        String sql =
                " INSERT INTO communication_type (comtyp_name)  VALUES (?); ";
        this.db.conectar();
        try {
     ...
     *
     */
    public void delete(CommunicationType o) {
        String sql =
                " DELETE FROM communication_type WHERE comtyp_id = ? ; ";
        this.db.conectar();
        try {
            pstmt = getConnection().prepareStatement(sql);
     ...
     * @return void
     */
    public void update(CommunicationType o) {
        String sql =
                " UPDATE communication_type SET comtyp_name = ? WHERE comtyp_id = ? ;";
        this.db.conectar();
        try {
            pstmt = getConnection().prepareStatement(sql);
            pstmt.setString(1, o.getName());
     ...
     */
    public CommunicationType get(int id, boolean allData) {
        CommunicationType temp = null;
        String sql = "";
        ...
        try {
            ...
            if (rs.next()) { // se retornou um número maior que 0 estão existe alguém que possui esse código
                ...
                temp = new CommunicationType(id, rs.getString("comtyp_name"));
            }
            rs.close();
            pstmt.close();
        } catch (SQLException e) {
     ...
     * @return
     */
    public ArrayList<CommunicationType> getList() {
     ...
     * quatidade Limit
     */
    public ArrayList<CommunicationType> getList(int begin, int limit) {
     ...
     * @param @return 
     */
    private ArrayList<CommunicationType> getList(int begin, int limit, boolean all) {
        ArrayList<CommunicationType> list = null;
        String sql = "";
        ...
        try {
            ...
            rs = pstmt.executeQuery();
            list = new ArrayList<CommunicationType>();
            while (rs.next()) { // se retornou um número maior que 0 estão existe alguém que possui esse código
                // Vai para o próximo retisro, no caso o 1º registro
                list.add(new CommunicationType(rs.getInt("comtyp_id"), rs.getString("comtyp_name")));
            }
            rs.close();
            pstmt.close();
        } catch (SQLException e) {
