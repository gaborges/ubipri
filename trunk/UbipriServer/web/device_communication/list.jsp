...
<html>
    <head>
        ...
        <title>DeviceCommunication List</title>
    </head>
    ...
    <body>
        <div class="container">
            ...
            <nav id="menu">
                <ul>
                    <li><h2>Device Communications</h2>
                        <ul>
            ...
            <div class="list">
                <table  id="tablepaging">
                    ...
                    <th><p>Name</p></th>
                    <th><p>Address</p></th>
                    <th><p>Address Format</p></th>
                    <th><p>Port</p></th>
                    <th><p>Parameters</p></th>
                    <th><p>Preferred Order</p></th>
                    <th><p>Communication Type</p></th>
                    <th><p>Update</p></th>
                    </tr>
                    </thead>
                    <form action="remove.jsp" method="get"  onSubmit="return confirmSubmit()">
                        <tbody>
                            <%
                                ...
                                DeviceCommunicationDAO deviceDAO = new DeviceCommunicationDAO(conexao);

                                ArrayList<DeviceCommunication> devices = deviceDAO.getList();
                            ...
                            <tr>
                                <td class="check"><input  type="checkbox" name="check" value="<%=device.getId()%>" /></td>
                                <td><%=device.getId()%></td>
                                <td><%=device.getName()%></td>
                                <td><%=device.getAddress()%></td>
                                <td><%=device.getAddressFormat()%></td>
                                <td><%=device.getPort()%></td>
                                <td><%=device.getParameters()%></td>
                                <td><%=device.getPreferredOrder()%></td>
                                <td><%=device.getCommunicationType().getName()%></td>
                ...
                <form name="form_incluir" action="form.jsp" class="inline">
                    <input type="submit" value="New DeviceCommunication" name="new"/>
                </form>
            </div>
        </div>
    </body>
